...
// the ElasticDetectionRule object against the actual cluster state, and then
...
func (r *ElasticDetectionRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	log := logf.Log.WithName("edr-watcher")
	if err := r.Get(ctx, req.NamespacedName, &edr); err != nil {
		log.Info("ElasticDetectionRule not found", "name", req.Name)
	...
	elk.Url = ""
	elk.Username = "foo"
	elk.Password = "" //pass this as a k8s secret

	log.Info("ElasticDetectionRule changed",
		"namespace", edr.Namespace,
		"name", edr.Name,
		"RuleName", edr.Spec.RuleName,
		"Description", edr.Spec.Description,
		"Type", edr.Spec.Type,
		"Enabled", edr.Spec.Enabled,
		"RiskScore", edr.Spec.RiskScore,
		"Severity", edr.Spec.Severity)
